...
from forecast_map_api import (
    ...
    build_wind_vectors, geo_shaded_traces, geo_contour_traces,
...
tab_15d, tab_35d = st.tabs(["📅 15 dias — ICON/GFS (determinístico)", "📆 GFS Ensemble (probabilístico)"])
...
if not grid_data:
    ...
    with tab_precip:
        ...
        for tr in geo_shaded_traces(z_p, "Blues", "mm", n_levels=8, vmin=0, vmax=vmax_p, alpha=0.3):
            fig_p.add_trace(tr)
    ...
    with tab_wind:
        ...
        for tr in geo_shaded_traces(z_w, "YlOrRd", "km/h", n_levels=8, vmin=0, vmax=vmax_w, alpha=0.3):
            fig_w.add_trace(tr)
    ...
    with tab_gusts:
        ...
        for tr in geo_shaded_traces(z_g, "OrRd", "km/h", n_levels=8, vmin=0, vmax=vmax_g, alpha=0.3):
            fig_g.add_trace(tr)
